...
Дописать объект в `js/builds.js`. Первый экран — каталог из 3 карточек: новая сборка встаёт
на место карточки «скоро». Фото для карточки — поле `image`, без него рисуется корпус.
Число мест — `CATALOG_SLOTS` в `js/app.js`. Когда сборок две и больше, на экране результата сама
